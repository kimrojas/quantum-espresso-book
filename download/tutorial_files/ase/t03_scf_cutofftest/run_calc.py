...
with open('run_calc.log', 'w') as f:
    ...
    for g in range(20, 120, 10):
        input_data['system']['ecutwfc'] = g
        input_data['system']['ecutrho'] = g*8

        calculator.set(input_data=input_data)
        calculator.set_label(label=label+f"_{g:03}")

        calculator.calculate(atoms)
        energy = atoms.get_potential_energy()
        # SAVE ENERGY
        subprocess.call(f"echo -e '{g:<4} {energy}' >> run_calc.dat", shell=True)
